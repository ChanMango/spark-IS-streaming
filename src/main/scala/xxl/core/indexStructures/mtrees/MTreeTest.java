...
public class MTreeTest {
	...
	public static Function SPHERE_COVERING_RECTANGLE_FACTORY = new AbstractFunction() {
		public Object invoke (Object kpe) {
			LabeledPoint center = getMidPoint((KPE)kpe);
	 ...
	 */
	public static LabeledPoint getMidPoint (KPE kpe) {
		...
		return new LabeledPoint(midValues);
	}
	...
	public MTreeTest (int id, int dim) {
		...
		CounterContainer lowerCounterContainer = new CounterContainer(
			new ConverterContainer(
				...
				new BlockFileContainer("/tmp/MTree_" + id, 46 * maxCapacity),

				// actually dimension of inserted points is '2'
				mTree.nodeConverter(mTree.leafEntryConverter(centerConverter(dimension)), 
						mTree.indexEntryConverter(descriptorConverter(dimension))) // define node converter
			)
		);

		// a buffered container that counts the access to the buffered MTree
		BufferedContainer bufferedContainer = new BufferedContainer(lowerCounterContainer, 
	...
	public void insert(float[] point) {
		Convertable cpoint = new LabeledPoint(point);
		tree.insert(cpoint);
	...
	public List<Pair<Double, Float>> kNNQuery(int k, float[] point) {
		...
		// (KPE ==> Sphere) to it
		Sphere queryObject = (Sphere)SPHERE_COVERING_RECTANGLE_FACTORY.invoke(new LabeledPoint(point));
		...
		Cursor cursor = new Taker(
			this.tree.query(new DynamicHeap(getDistanceBasedComparator(queryObject)), targetLevel),
			k
		);
