...
class StreamingDistributedKNN (    
  ...
  protected var model: StreamingDistributedKNNModel = new StreamingDistributedKNNModel(null, EMPTY, 0.0)
  protected var subTrees: DStream[(Int, MTreeWrapper)] = null
  protected var lastTime: Time = null
  ...
  def trainOn(data: DStream[LabeledPoint]) {
    ...
    var btree = model.topTree
    var newModel: StreamingDistributedKNNModel = null
    var kg = kGraph
    var remove = removeOld
    var nt = nTrees
    
    data.foreachRDD{ rdd => 
      if(model.topTree == null) {   
        // There is no master tree created yet
        model = initializeMaster(rdd)
        btree = model.topTree
      }
    }
    //model = newModel 
    /*val unbalanced = false
    subTrees.foreachRDD{ rdd => 
      if(isUnbalanced(rdd)){ // No re-balance for static experiments 
          logInfo("Re-balancing the distributed m-tree." + 
              "One or more sub-trees have grown too much.")  
          val casebase = rdd.values.flatMap{ tree =>
            tree.getIterator.map(jlp => LPUtils.fromJavaLP(jlp)) 
          }
          model = initializeMaster(casebase)
      }
    }
    
    subTrees = subTrees.transform{ rdd => 
      if(unbalanced){ // No re-balance for static experiments      
          val casebase = rdd.flatMap{ case (idx, tree) =>
            tree.getIterator.map(jlp => LPUtils.fromJavaLP(jlp)) 
          }
          initializeTrees(casebase)
      } else {
          rdd
      }
    }*/
    ...
    //Update process
    val indexedData = data.map{ lp => 
      val indices = searchIndex(lp.features, btree.value)
      assert(indices != -1, s"indices must be non-empty: $lp")
      indices -> lp
    }
    
    val toedit = edited
    val updateFunction = (iterator: Iterator[(Int, Seq[LabeledPoint], Option[MTreeWrapper])]) => { 
      iterator.map{ case(key, newValues, tree) =>
        ...
        if(toedit && !firstBatch) {
          newValues.foreach { point =>
            val lp = new TreeLP(point, key, null)
            val neighbors = newTree.kNNQuery(kg, point.features.toArray.map(_.toFloat))
              .map(pair => new TreeLP(LPUtils.fromJavaLP(pair.data.asInstanceOf[DataLP]), key, null))
              .toArray   
            val edited = InstanceSelection.localInstanceSelection(lp -> neighbors, remove)
            edited.foreach{ lp => 
              if(lp.action == INSERT)
                newTree.insert(LPUtils.toJavaLP(lp.point))
              else if (lp.action == REMOVE)
                newTree.remove(LPUtils.toJavaLP(lp.point))
            }            
          
          }
        } else {
          newValues.foreach { point =>
            newTree.insert(LPUtils.toJavaLP(point))
          }
        }        
        key -> newTree        
      }      
    ...
    /*val mappingFunction = (key: Int, newValue: Option[LabeledPoint], state: State[MTreeWrapper]) => {
      // Use state.exists(), state.get(), state.update() and state.remove()
      // to manage state, and return the necessary string
      val newTree = state.getOption() match {
        case Some(tree) => 
          newValue match {
            case Some(point) => 
              if(edited) {
                ...
                tree.insert(LPUtils.toJavaLP(point))
              }              
              tree
            case None =>
              tree
      ...
      state.update(newTree)
      None
    }
    val spec = StateSpec.function(mappingFunction).partitioner(new HashPartitioner(nTrees))
    val mapWithStateDStream = indexedData.mapWithState(spec)*/
    subTrees = indexedData.updateStateByKey(updateFunction, new HashPartitioner(nt), rememberPartitioner = true).checkpoint(new Duration(10000))
    //subTrees = st
    subTrees.foreachRDD((rdd: RDD[(Int, MTreeWrapper)], time: Time) => if(!rdd.isEmpty()) lastTime = time else lastTime = null)
  }
  
  private def isUnbalanced(subTrees: RDD[(Int, MTreeWrapper)]) = {
    if(!subTrees.isEmpty()){
      !subTrees.filter(_._2.getSize < kGraph * 2).isEmpty()
    } else {
      false
    }
  }
  
  private def initializeMaster(rdd: RDD[LabeledPoint]) = {
      // Initialize topTree (first registers coming...)
      val rand = new XORShiftRandom(this.seed + 2)
      val nt = nTrees
      val firstLoad = rdd.takeSample(false, nt, rand.nextLong())
      ...
      new StreamingDistributedKNNModel(bTopTree, NEW, tau)
  }
  
  private def initializeTrees(rdd: RDD[LabeledPoint]) = {
      
      // Load the instances in the sub-trees
      val btree = model.topTree
      val repartitioned = rdd.map(v => (v, null)).partitionBy(new KNNPartitioner(btree))
      val trees = repartitioned.mapPartitions { itr =>
        val childTree = new MTreeWrapper(itr.map(t => LPUtils.toJavaLP(t._1)).toArray)
        Iterator(childTree)
      }.zipWithIndex.map{ case(tree, idx) => idx.toInt -> tree}
  ...
  private def fastkNNQuery(data: RDD[LabeledPoint], trees: RDD[(Int, MTreeWrapper)], k: Int): RDD[(TreeLP, Array[TreeLP])] = {
      val btree = model.topTree
      val partitioner = new HashPartitioner(nTrees)
      val searchData = data.map { vector =>
        val indices = searchIndex(vector.features, btree.value)
  ...
  /*private def accuratekNNQuery(data: DStream[LabeledPoint], k: Int): RDD[(TreeLP, Array[TreeLP])] = {

    // Efficient join if elements and their neighbors have the same partitioning scheme
    val indexedData = data.zipWithIndex.map(_.swap).partitionBy(new HashPartitioner(nTrees)).cache
    val searchData = indexedData.flatMap { case (idx, vector) =>
        val indices = searchIndices(vector.features, model.topTree.value, model.tau)
          .map(i => (i, (vector, idx)))
        assert(indices.filter(_._1 == -1).length == 0, s"indices must be non-empty: $vector")
        indices
    }.partitionBy(new HashPartitioner(nTrees))
    ...
    indexedData.join(neigs).values.map{ case(lp, neigs) =>
      val tlp = new TreeLP(lp, searchIndex(lp.features, model.topTree.value), null)
      tlp -> neigs
    }

  }*/
  
  /*def predict(data: RDD[LabeledPoint], trees: RDD[MTreeWrapper]): RDD[(Double, Double)] = {
    if(model.topTree != null && !data.isEmpty()) {
      kNNQuery(data, trees, k).map{ case (lp, arr) =>
   ...
   */
  def predictOnValues2(data: DStream[LabeledPoint], k: Int): DStream[(Double, Double)] = {
    //assertInitialized()
    val predict = (d: RDD[LabeledPoint], t: RDD[(Int, MTreeWrapper)]) => {
      //if(model.topTree != null && t != null && !d.isEmpty()) {
        val neighbors = fastkNNQuery(d, t, k)
        //if(model.tau > 0) accuratekNNQuery(data, k) else fastkNNQuery(data, k)
        neighbors.map{ case (lp, arr) =>
          val pred = arr.map(_.point.label).groupBy(identity).maxBy(_._2.size)._1
          (lp.point.label, pred)
        }   
      //} else {
      //  d.context.emptyRDD[(Double, Double)]
      //}
    }
    
    data.transformWith(subTrees, predict)
  }
  
  def predictOnValues(data: DStream[LabeledPoint], k: Int): DStream[(Double, Double)] = {
    //assertInitialized()
    val predict = (d: RDD[LabeledPoint]) => {
      if(lastTime != null) {
        val t = subTrees.compute(lastTime).get
        val neighbors = fastkNNQuery(d, t, k)
        //if(model.tau > 0) accuratekNNQuery(data, k) else fastkNNQuery(data, k)
        neighbors.map{ case (lp, arr) =>
          val pred = arr.map(_.point.label).groupBy(identity).maxBy(_._2.size)._1
...
object StreamingDistributedKNN {
  ...
  trait BaseAction
  case object CREATED extends BaseAction
  case object EMPTY extends BaseAction
  case object NEW extends BaseAction
  case object REINITIALIZED extends BaseAction
